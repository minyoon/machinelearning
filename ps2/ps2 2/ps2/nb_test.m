...
for i=1:numTestDocs
    ...
    for j=1:numTokens
        scoreSpam = scoreSpam+log(condProbSpam(j))*testMatrix(i,j);
        scoreHam = scoreHam+log(condProbHam(j))*testMatrix(i,j);
    end
...
fifthProb=0;
for j=1:numTokens
    prob=log(condProbSpam(j)/condProbHam(j));
    if prob>highestProb
        highestProb=prob;
    elseif prob>secondProb
        secondProb=prob;
    elseif prob>thirdProb
        thirdProb=prob;
    elseif prob>fourthProb
        fourthProb=prob;
    elseif prob>fifthProb
        fifthProb=prob;
    end
end
%---------------
